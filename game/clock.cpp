...
Clock::Clock() :
  ...
  frameRate(0.0),
  frameRates(new unsigned int[Gamedata::getInstance().getXmlInt("frameMax")+1])
  {
  for(int i = 0; i < Gamedata::getInstance().getXmlInt("frameMax")+1; i++) frameRates[i]=0;
  start();
}
...
void Clock::display() const {
  ...
  IOManager::getInstance()
    .printMessageValueAt("fps ", frameRate, 10, 50);
...
unsigned int Clock::getElapsedTicks() {
  ...
  int max = Gamedata::getInstance().getXmlInt("frameMax");
   tickSum -= frameRates[frames%max];
   frameRates[frames%max] = ticks;
  tickSum += ticks;
   frameRate = (float)((int)frames < max?frames : max) / (tickSum / 1000.0); //rate = frames / ticks
  frameRates[frames%max] = ticks;
  return ticks;
}
